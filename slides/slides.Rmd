...
\caption{Adjacency matrices of (from left to right) the British MPs Twitter network (Greene and Cunningham, 2013), Political Blogs network (Adamic and Glance, 2005), and DBLP network (Gao et al., 2009) after sorting by the clustering outputted by OSC.}\label{fig:mp}
\end{figure}
...
* Published in JCGS.
* Code, data, and preprint drafts available at https://github.com/johneverettkoo/pabm-grdpg
* R package WIP at https://github.com/johneverettkoo/osc
...
1. Draw $z_1, ..., z_n \iid \Categorical(1/2, 1/2)$.
...
1. Draw $z_1, ..., z_n \iid \Categorical(1/3, 1/3, 1/3)$.
2. Draw $t_1, ..., t_n \iid \Uniform(0, 1)$. 
...
```{r mbconnectome-kcurves, fig.height = 2, fig.width = 8, cache = TRUE, out.width = '100%', fig.cap = 'Left and right hemispheres of the Drosophila connectome (top), ASEs (middle), and fitted curves (bottom).'}
set.seed(123456)
...
## Expected Degree and Sparsity Factor

Let $d_1, ..., d_n$ be the degrees of vertices $v_1, ..., v_n$. 
For $P$ defined by a block model or GRDPG, $E[d_i] = O(n)$. 
In most real networks, the degree does not scale linearly with the size of the network. 

Introduce a sparsity factor $\rho_n$ and draw $A \sim \BG(\rho_n P)$.

* Sparse regime: $\lim\limits_{n \to \infty} \rho_n \to 0$
* Dense regime: $\rho_n = 1$

Most results still require $n \rho_n = \omega (\log^c n)$, i.e., 
expected degree still diverges, but at a logarithmic rate instead of linearly. 
...
```{r, cache = TRUE, fig.height = 3, fig.width = 4, out.width = '50%'}
set.seed(314159)
...
## Fitting Bezier Curves to Noisy Data

Let $g(t; p) = \sum_{r=0}^R p^{(r)} \binom{R}{r} t^r (1-t)^{R-r}$ be a Bezier curve of order $R$ with coefficients $p = [p^{(0)} \mid \cdots \mid p^{(R)} ]^\top$. 

Objective: minimize squared loss: $L(p, T; X) = \|X - T p\|^2$.

$T$ is the $n \times (R+1)$ matrix of index points, i.e., $T_{ir} = \binom{R}{r} t_i^r (1-t_i)^{R-r}$.

If we know $T$, we can fit $p$: $\hat{p} = (T^\top T)^{-1} T^\top X$.

If we know $p$, we can fit each $t_1, ..., t_n$ by finding the roots of $\dot{L}(t_i; p, x_i) = \bigg(\sum_{r=0}^R \binom{R}{r} (-1)^r c_r t^r \bigg) \bigg(\sum_{r=0}^{R-1} \binom{R-1}{r} (-1)^r c_{r+1} t^r \bigg)$, where $c_R = \sum_{r=0}^R (-1)^{R-r} \binom{R}{r} p^{(r)}$.

## ASE Clustering for Nonintersecting Communities
...
```{r two-curves, fig.cap = 'Manifold block model from two disjoint curves. The latent configuration is on the left, a random dot product graph drawn from the latent configuration is on the middle, and the ASE is on the right.', fig.height = 2, fig.width = 8, out.width='100%'}
set.seed(123456)
n1 <- 2 ** 5
n2 <- 2 ** 5
n <- n1 + n2
...
**Theorem** (KTT): Let $A$ be a graph on $n$ vertices from the MBM model in wherein $n \rho_n = \omega(\log^{c}{n})$ for some $c > 1$ and the manifolds $\mathcal{M}_1, \dots, \mathcal{M}_{K}$ satisfy $\min_{k \neq \ell} \min_{x \in \mathcal{M}_k, y \in \mathcal{M}_{\ell}} \|x  - y\| \geq \delta$ for some $\delta > 0$ not depending on $n$ or $K$. 
...
* Adamic, L. A. and Glance, N. (2005). The political blogosphere and the 2004 U.S. election: Divided they blog. In *Proceedings of the 3rd International Workshop on Link Discovery*, LinkKDD '05. 
* Banerjee, A., Chandrasekhar, A. G., Duflo, E., and Jackson, M. O. (2013). The diffusion of Microfinance. Harvard Dataverse. 
* Eichler, K., Li, F., Litwin-Kumar, A., Park, Y., Andrade, I., Schneider-Mizell, C. M., Saumweber, T., Huser, A., Eschbach, C., Gerber, B., Fetter, R. D., Truman, J. W., Priebe, C. E., Abbot, L. F., Thum, A. S., Zlatic, M., and Cardona, A. (2017). The complete connectome of a learning and memory center in an insect brain. *bioRxiv*.

## References 

* Elhamifar, E. and Vidal, R. (2009). Sparse Subspace Clustering. *2009 IEEE Conference on Computer Vision and Pattern Recognition*. 
...
* Greene, D. and Cunningham, P. (2013). Producing a unified graph representation from multiple social network views. *CoRR*. 
* Karrer, B. and Newman, M. E. J. (2011). Stochastic blockmodels and community structure in networks. *Physical Review E*. 
* Koo, J., Tang, M., and Trosset, M. W. (2022). Popularity adjusted block models are generalized random dot product graphs. *Journal of Computational and Graphical Statistics*. 

## References

* Lorrain, F., and White, H. C. (1971). Structural equivalence of individuals in social networks. *The Journal of Mathematical Society*. 
* Nepusz, T., Petroczi, A., Negyessy, L, and Bazso, F. (2008). Fuzzy communities and the concept of bridgeness in complex networks. *Phys. Rev. E*. 
* Noroozi, M. and Pensky, M. (2022). The hierarchy of block models. *Sankhya A*.
* Noroozi, M., Rimal, R., and Pensky, M. (2019). Estimation and clustering in the popularity adjusted block model. *Journal of the Royal Statistical Society: Series B (Statistical Methodology)*.

## References

* Negyessy, L., Nepusz, T., Kocsis, L., and Bazso, F. (2006). Prediction of the main cortical areas and connections involved in the tactile function of the visual cortex by network analysis. *European Journal of Neuroscience*. 
...
* Sengupta, S. and Chen, Y. (2018). A block model for node popularity in networks with community structure. *Journal of the Royal Statistical Society, Series B (Statistical Methodology)*. 
* Sussman, D. L., Tang, M., Fishkind, D. E., and Priebe, C. E. (2012). A consistent adjacency spectral embedding for stochastic blockmodel graphs. *Journal of the American Statistical Association*. 

## References

* Wang, Y.-X. and Xu, H. (2016). Noisy sparse subspace clustering. *Journal of Machine Learning Research*. 