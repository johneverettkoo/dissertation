...
$$\ell(z, \theta; A) = \sum_{i<j} \sum_{k, \ell} z_{ik} z_{j \ell} \Big(A_{ij} \log_{k \ell} + (1 - A_{ij}) \log(1 - \theta_{k \ell}) \Big)$$
...
**Theorem** (KKT): Let $A \sim \MBM(\{\alpha_1, ..., \alpha_K\}, \{F, ..., F\}, \{g_1, ..., g_K\}; \rho_n)$ such that $F$ has support $[0, 1]$, and each $g_k(t) = g(t; p_k)$ is is a curve of order $R$ that does not self-intersect (for any $s \neq t$, $g_k(s) \neq g_k(t)$). 
