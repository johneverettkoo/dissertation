---
title: Community Detection Methods for Generalized Random Dot Product Graphs
...
author: 
  ...
  - Fangzheng Xie$^1$
institute:
  - $^1$Indiana University, Department of Statistics
  - $^2$North Carolina State University, Department of Statistics
  - $^3$Indiana University, Department of Computer Science
date: 'June 2021'
output: 
  beamer_presentation:
    fig_crop: no
...
1. Note that $A \sim \BG(P)$ is a RDPG since $P = X X^\top$.
...
**Theorem** (KTT): 
Given popularity vectors $\{\lambda^{(kl)}\}_2$, we can define block diagonal $X \in \mathbb{R}^{n \times 4}$ and $U \in \mathbb{O}(4)$ such that $A \sim \PABM(\{\lambda^{(kl)}\}_2)$ is equivalent to $A \sim \GRDPG_{3, 1}(X U)$.
...
\hat{v}_i = \hat{V} c \text{ and } c^{(i)} = 0$$
...
1. $r = 1$ and each $\gamma_k$ is a point mass $\implies$ SBM
2. $r = 1$ and the range of each $\gamma_k$ is a segment from the origin to point $y_k$ with $\|y_k\| \leq 1$ $\implies$ DCBM
3. $r = K$ and the range of each $\gamma_k$ is the hyperplane of coordinates $x_{K (k-1) + 1}$ to $x_{k K}$ $\implies$ PABM
...
This leads to a general community detection method:
...
$B_{ij} = 0$ if $x_i$ and $x_j$ belong to different subspaces.

::: columns

:::: column

```{r, fig.height = 5, fig.width = 5, out.width = '75%'}
set.seed(314159)
...
* $Z_i \iid Categorical(\pi_1, ..., \pi_K)$
* $\lambda_{ik} \indep Beta(a_{ik}, b_{ik})$
