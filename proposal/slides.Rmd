...
Example 1: Stochastic Block Model with two communities
...
Example 1 (cont'd): Assortative SBM ($p q > r^2$) with $K = 2$
...
If we want to perform community detection, 

1. Note that $A \sim BernoulliGraph(X X^\top)$ is a RDPG
2. Compute the ASE $A \approx \hat{X} \hat{X}^\top$  
...
**Theorem** (KTT): 
Given popularity vectors $\{\lambda^{(kl)}\}_2$, we can define block diagonal $X \in \mathbb{R}^{n \times 4}$ and $U \in \mathbb{O}(4)$ such that $A \sim PABM(\lambda^{(kl)}_2)$ is equivalent to $A \sim GRDPG(X U)$.
...
If $A \sim GRDPG_{p, q}(X)$, then $A \sim GRDPG_{p, q}(X Q)$ for any $Q$ in 
the indefinite orthogonal group with signature $p, q$.

## Sparse Subspace Clustering

**Corollary**: Since $X$ is block diagonal by community and $U$ is orthogonal,
...
qgraph::qgraph(sign(W), layout = 'spring', groups = factor(z))
```
