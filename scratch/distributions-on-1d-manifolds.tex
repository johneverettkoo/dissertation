...
\begin{theorem}
Let $X_1, ..., X_n \stackrel{\mathrm{iid}}{\sim}F$ with support $[0, 1]^r$, and $f(x) \geq a > 0$ everywhere on the support. Define $E_n$ as the event that every point on the support is within distance $\delta / 2$ to $X_i$ for $i \in [n]$. Then for any $\epsilon > 0$, there exists $N = O(-\log \epsilon)$ such that $P(E_n) > 1 - \epsilon$ when $n \geq N$.
\end{theorem}
