...
where $T = \begin{bmatrix} t^{(1)} & \cdots & t^{(R)} \end{bmatrix}$ and each $t^{(r)}_i = \binom{R}{r} (1 - t_i)^{R - r} t_i^r$, and $X \in \mathbb{R}^{n \times d}$. 
...
In the case $\Sigma(t_i | \phi) = \sigma^2 I$ and $f(t_i | p) = p^\top t_i$ where $t_i \in \mathbb{R}^{R+1}$ is the Bezier polynomial expansion of order $R$, then this becomes

$$\ell(p, \sigma^2) = -\frac{n d}{2} \log \sigma^2 - \frac{1}{2 \sigma^2} \|X - T p\|^2_F$$

Then given $t_1, ..., t_n$, the MLE of $p$ is again $(T^\top T)^{-1} T^\top X$, and the MLE of $\sigma^2$ is just $\frac{1}{n d} \|X - T \hat{p}\|^2_F$.
And given $p$, we can estimate each $t_i$ in the same way as before to maximize $\ell$ ($\sigma^2$ is unnecessary here). 
...
The CLT property of the ASE may allow us to approximate $W x - \hat{x} \sim \mathcal{N}(f_k(t), \Sigma_k)$.
One possibly reasonable assumption for $\Sigma_k$ is $\Sigma_k = \Sigma_k(t) =  \diag(\sigma_{k1}^2(t), ..., \sigma_{kd}^2(t))$. A plot of the squared errors of cluster 1 in the previous example reveals the following:

```{r, fig.height = 2, fig.width = 8}
theta1 <- manifold.out$theta[[1]]
theta2 <- manifold.out$theta[[2]]
t. <- manifold.out$t
X1 <- param.curve.quadr.2(t.[[1]], 
                          theta1[1], theta1[2], theta1[3],
                          theta1[4], theta1[5], theta1[6])
X2 <- param.curve.quadr.2(t.[[2]], 
                          theta2[1], theta2[2], theta2[3],
                          theta2[4], theta2[5], theta2[6])
# plot(Xhat[zhat.manifold == 1, 1] - X1[, 1])
# plot(Xhat[zhat.manifold == 1, 2] - X1[, 2])

p1 <- ggplot() + 
  geom_point(aes(x = t.[[1]], y = (Xhat[zhat.manifold == 1, 1] - X1[, 1]) ^ 2)) + 
  stat_smooth(aes(x = t.[[1]], y = (Xhat[zhat.manifold == 1, 1] - X1[, 1]) ^ 2)) + 
  theme_bw() + 
  labs(x = 't', y = expression((x[1] - hat(x)[1])^2))

p2 <- ggplot() + 
  geom_point(aes(x = t.[[1]], y = (Xhat[zhat.manifold == 1, 2] - X1[, 2]) ^ 2)) + 
  stat_smooth(aes(x = t.[[1]], y = (Xhat[zhat.manifold == 1, 2] - X1[, 2]) ^ 2)) + 
  theme_bw() + 
  labs(x = 't', y = expression((x[2] - hat(x)[2])^2))

gridExtra::grid.arrange(p1, p2, nrow = 1)
```

Fitting some curve to each $\sigma_{kr}^2(t)$ allows us to compute estimated probabilities of each point belonging to each cluster/manifold: $\hat{q}_{ik} = \mathcal{N}(x_i \mid g_k(\hat{t}_i), \Sigma_k(\hat{t}_i))$. 
Then we can use weighted least squares to estimate the parameters of $g$:

$$\hat{p}_k = (T^\top Q_k T)^{-1} T^\top Q_k X$$

where $Q_k = \diag(q_{1k}, ..., q_{nk})$.